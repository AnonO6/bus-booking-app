  ...
  return (
    <View
      ...
      <View style={{paddingLeft: 10, paddingRight: 10, marginTop: 15}}>
        ...
        <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
          <Text style={{fontSize: 10, color: GlobalBackgroundTextColors}}>
            {Bus.Day}, {Bus.Date}
          </Text>
          <Text style={{fontSize: 10, color: GlobalBackgroundTextColors}}>
            {Bus.DepartureTime}
      ...
      <TouchableOpacity
        style={primaryButton}
        onPress={() =>
          navigation.navigate('RefundTicketScreen', {Bus, person, selectedData})
        }>
        <Text
          style={{
            color: 'white',
  ...
  useEffect(() => {
    ...
    BusNode.once('value')
      ...
      .then(readCountTxn => {
        ...
        // Getting pending related data
        duppendingBookingBus = FilterBusSchedule(
          scheduleBuses,
          person.PhoneNumber,
        );
