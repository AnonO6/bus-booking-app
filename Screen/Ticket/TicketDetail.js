...
function TicketDetail({navigation, route}) {
  ...
  const AccountModalOkHandler = setError => {
    ...
    if (Bus.bookedIndex == -1) {
      // Means there is no booking for the user so we have to get all the booking and then insert the booking of user as well
      newBookedSeats = [
        {PhoneNumber: person.PhoneNumber, seats: [...Bus.Pending]},
      ];
      ...
      Node.once('value')
        ...
        .then(readCountTxn => {
          ...
          Node = rootReference.child(
            'BusSchedule/' + Bus.FireBaseIndex + '/Booked',
          );
      ...
      updateNode = rootReference.child(
        'BusSchedule/' +
          Bus.FireBaseIndex +
          '/Booked/' +
          Bus.bookedIndex +
  ...
  return (
    <View
      style={{backgroundColor: GlobalBackgroundColors.primaryColor, flex: 1}}>
      {promise ? (
        <ScrollView style={{flex: 1}}>
          ...
          <View
            ...
            <AccountModel
              ...
              AccountModalOkHandler={AccountModalOkHandler}
              AccountCancelHandler={() => AccountCancelHandler()}
              setAccountNumber={setAccountNumber}
            />
          </View>
          <View style={{display: isPurchasedDoneVisible ? 'flex' : 'none'}}>
            <PurchasedDoneModel
          ...
          <View
            ...
            <View
              ...
              <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                <TouchableOpacity
                  style={[primaryButton, globalShadowBox, {height: 50}]}
