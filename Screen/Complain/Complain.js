        ...
        {radio_props.map((obj, i) => (
          <RadioButton labelHorizontal={true} key={i}>
            <RadioButtonInput
              ...
              buttonOuterSize={15}
              buttonInnerColor={
                obj.value === props.radioData
                  ? GlobalBackgroundColors.primaryColor
                  : 'white'
              }
    ...
    if (textInput.length <= 10) {
      ...
      complainNode
        ...
        .then(readCountTxn => {
          allComplains = [
            ...allComplains,
            {PhoneNumber: person.PhoneNumber, Message: textInput},
          ];
    ...
    <View style={{flex: 1}}>
      {promise ? (
        <SimpleContainer isBottomVisible={true} headerFlexSize={20}>
          {/* Complain */}
          <View style={{display: iscomplainVisible ? 'flex' : 'none'}}>
            <ComplainModal
              isVisible={iscomplainVisible}
              ComplainDone={messageReceived}
            />
          </View>
          ...
          <View style={{marginTop: 20}}>
            <TouchableOpacity
              style={[primaryButton]}
              onPress={() => ComplainDone()}>
              <Text
                style={{
                  textAlign: 'center',
                  color: GlobalBackgroundColors.ternaryColor,
                }}>
                Submit
              </Text>
            </TouchableOpacity>
