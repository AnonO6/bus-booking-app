    ...
    <View style={{flex: 1}}>
      ...
      <View
        ...
        <View
          style={{
            height: 5,
            backgroundColor: GlobalBackgroundTextColors.primaryColor,
            width: 30,
            marginTop: 'auto',
            marginBottom: 'auto',
          }}></View>
...
function Bus(props) {
  ...
  return (
    <TouchableOpacity
      onPress={() =>
        props.navigation.navigate('BookingBusSeatsScreen', {person, Bus})
      }
      style={[
        globalShadowBox,
        {
...
function BookingBuses({navigation, route}) {
  /*===== Getting Passed Data ===================================================================== */
  var {ArrivalSelected, DestinationSelected, DateData, person, DateSchedule1} =
    route.params;
  ...
  var AllBuses = BusSchedule.filter((bus, index) => {
    if (
      bus.Date == Date &&
      bus.Destination == DestinationSelected &&
      bus.Arrival == ArrivalSelected
    ) {
      return bus;
    }
  });
