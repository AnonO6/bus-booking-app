    ...
    <Modal transparent={true} visible={props.isVisible} style={{width: '100%'}}>
      <View
        ...
        <View
          ...
          <View style={{height: 15, marginTop: 50, alignItems: 'center'}}>
            <Text style={{color: 'red'}}>{error}</Text>
          </View>
          <TextInput
            style={[styles.textInputStyle, focusStyle, {width: '100%'}]}
            onChangeText={text => {
              props.setAccountNumber(text);
            }}
            value={props.value}
            placeholder="Enter LNMIIT bus card number"
            onFocus={() => setFocusStyle({outline: 'none'})}
          />
          <View style={{marginTop: 10}} />
          <Button
            title="     OK     "
            onPress={() => props.AccountModalOkHandler(setError)}
          />
          <View style={{marginTop: 10}} />
